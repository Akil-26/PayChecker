...
class PaycheckerError(Exception):
...
class RecordNotFound(PaycheckerError):
...
class InvalidStateTransition(PaycheckerError):
...
class CaseAlreadyTerminal(PaycheckerError):
...
class ActionNotDue(PaycheckerError):
...
class UnsupportedAction(PaycheckerError):
...
class DemoResetForbidden(PaycheckerError):
    """The demo/seed reset endpoint was called outside a development environment.

    Reachable in production, this endpoint would let anyone erase the audit trail
    by dropping and recreating every table, so it is blocked outright rather than
    ...
    def __init__(self, environment: str, allowed: tuple[str, ...]) -> None:
        super().__init__(
            f"Demo reset is not permitted in '{environment}' environment. "
            "This endpoint is only available in: " + ", ".join(sorted(allowed)) +
            ". It is blocked in production to protect the audit trail."
        )
