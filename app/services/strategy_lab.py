...
class StrategyComparison:
    """Baseline strategy against Paychecker's expected-value selection."""
...
class StrategyLabService:
    ...
    def _reason(
        ...
        sentence = (
            f"Paychecker recommends {recommended} because it maximises expected "
    ...
    def baseline_comparison(self, *, limit: int | None = None) -> BaselineComparison:
        ...
        Paychecker = self._project(cases, strategy="Paychecker")

        recovered_uplift = Paychecker.projected_recovered - baseline.projected_recovered
        ...
        return BaselineComparison(
            ...
            recovery_rate_uplift_pct=round(
                (Paychecker.projected_recovery_rate - baseline.projected_recovery_rate) * 100,
                2,
            ),
            expected_value_uplift=(
                Paychecker.expected_recovery_value - baseline.expected_recovery_value
            ),
    ...
    def _project(self, cases: list[RecoveryCase], *, strategy: str) -> StrategyComparison:
        ...
        return StrategyComparison(
            strategy="Baseline" if strategy == "baseline" else "Paychecker",
            description=(
