...
class AutopilotService:
    ...
    def _run_case(self, case: RecoveryCase) -> AutopilotCaseResult:
        ...
        for index in range(1, MAX_RUNS_PER_CASE + 1):
            ...
            except PaycheckerError as failure:
            ...
            # In development: advance the virtual clock so the scheduled retry becomes due.
            # In production: real time passes on its own — just re-run immediately
            # (the WallClock.advance() is a no-op, but is_due() will return True
            # once real wall time has passed the scheduled_at, so we re-run and
            # let the workflow decide if it is due yet).
            if run.waiting:
                from app.core.clock import VirtualClock
                if isinstance(self._clock, VirtualClock):
                    self._clock.advance(minutes=self._settings.retry_later_delay_minutes)
                    clock_advances += 1
                # In production (WallClock), do not advance — the workflow will
                # return waiting=True until real time reaches scheduled_at.
                # Break to avoid a tight infinite loop; the caller should
                # re-run the autopilot after the real delay has elapsed.
                else:
                    break
